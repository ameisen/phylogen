...
class scoped_clamp
{
   ...
   T max;
public:
   scoped_clamp(T &val, T _min, T _max) : value(val), min(_min), max(_max)
   {
   }
...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ...
    if (flags & ImGuiButtonFlags_Disabled)
    {
        ...
        if (flags & ImGuiButtonFlags_Current_PC)
        {
           *out_hovered = true;
        }
        return false;
...
{
   scoped_clamp<float> _clamp(*v, v_min, v_max);
...
{
   scoped_clamp<int> _clamp(*v, v_min, v_max);
...
void ImGui::ProgressBar(float fraction, const ImVec2& size_arg, const char* overlay)
{
    ...
    if (overlay_size.x > 0.0f)
        RenderTextClipped(ImVec2(ImClamp(style.ItemSpacing.x, bb.Min.x, bb.Max.x - overlay_size.x - style.ItemInnerSpacing.x), bb.Min.y), bb.Max, overlay, NULL, &overlay_size, ImGuiAlign_Left|ImGuiAlign_VCenter, &bb.Min, &bb.Max);
}

bool ImGui::Checkbox(const char* label, bool* v, bool hide_label)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(hide_label ? "" : label, NULL, true);
    ...
    if (label_size.x > 0.0f)
        RenderText(text_bb.GetTL(), hide_label ? "" : label);
