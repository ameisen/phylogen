...
PInput main(VInput input)
{
   ...
   float radius = lerp(ThisInstanceData.Radius, ThisInstanceData.Armor_NucleusScale.y, WorldRadius_Lit_NuclearScaleLerp.z);

   const float targetRadius = radius * Scale_ColorLerp_MoveForward.x;
   ...
   float2 forward = float2(1.0, 0.0) * (Scale_ColorLerp_MoveForward.w * targetRadius);

   output.Position = mul(float4(((input.Position / 1.2) * targetRadius) + forward, 0.0, 1.0), MVP);

   output.Position.z = 0.5;

   output.InstanceID = input.InstanceID;

   output.SV_Position = float4((((input.Position / 1.2) + 1.0) / 2.0) * Scale_ColorLerp_MoveForward.x, 0.0, 1.0);
